...
class TaskSamplerBase(object):
    ...
    def get_scene_entities(self) -> None:
        ...
        for e_id in self.sg.object:
            ...
            if parent_room_id is not None:
                ...
                if parent_room_id not in self.room_names:
                    ...
                    self.place_names[self.num_places] = place_to_str_name(
                        self.num_places, parent_room, is_room=True
                    )
                    self.num_places += 1
                ...
                if scene_entity.class_ in RECEPTACLES:
                    ...
                    self.receptacles["class_count"][receptacle_class] = (
                        self.receptacles["class_count"].get(receptacle_class, 0) + 1
                    )
                    ...
                    if scene_entity.class_ in OPENING_RECEPTACLES:
                        self.receptacles["opening_type"].add(e_id)
                    if scene_entity.class_ in HEATING_RECEPTACLES:
                        self.receptacles["heating_type"].add(e_id)
                    if scene_entity.class_ in COOLING_RECEPTACLES:
                        self.receptacles["cooling_type"].add(e_id)
                    if scene_entity.class_ in CLEANING_RECEPTACLES:
                        self.receptacles["cleaning_type"].add(e_id)

                elif scene_entity.class_ in OBJECTS:
                    self.objects["all"].add(e_id)
                    object_class = scene_entity.class_.replace(" ", "")
                    self.objects["class_count"][object_class] = (
                        self.objects["class_count"].get(object_class, 0) + 1
                    )
                    ...
                    self.object_names[e_id] = object_to_str_name(
                        scene_entity, self.object_sizes[e_id]
                    )

        self.num_objects = len(self.objects["all"])
        self.num_receptacles = len(self.receptacles["all"])
        self.num_rooms = len(self.room_names)
        if self.num_objects == 0 or self.num_receptacles == 0 or self.num_rooms == 0:
            self.valid_scene = False
        if len(self.objects["heatable_type"]) > 0 and len(self.receptacles["heating_type"]) > 0:
            self.can_heat = True
        if len(self.objects["coolable_type"]) > 0 and len(self.receptacles["cooling_type"]) > 0:
            self.can_cool = True
        ...
        sorted_receptacle_class = sorted(
            list(self.receptacles["class_count"].items()), key=lambda x: x[1]
        )
        ...
        for room_id in self.sg.room:
            if room_id not in self.room_names:
                ...
                self.place_names[self.num_places] = place_to_str_name(
                    self.num_places, room, is_room=True
                )
                self.num_places += 1
    ...
    def get_support_relations(self, dist_threshold: float = 2) -> None:
        ...
        for o_id in object_distances:
            ...
            self.lifted_class_relations.add(
                (
                    self.sg.object[o_id].class_.replace(" ", ""),
                    self.sg.object[r_id].class_.replace(" ", ""),
                )
            )
        ...
        for o_id in self.unsupported_objects:
            obj_inst = self.sg.object[o_id]
            self.place_names[self.num_places] = place_to_str_name(
                self.num_places, obj_inst, is_object=True
            )
    ...
    def get_locations(self) -> None:
        ...
        for o_id in self.objects["all"]:
            ...
            location_name = location_to_str_name(
                room_data, self.entity_to_place_map["objects"][o_id]
            )
        ...
        for r_id in self.receptacles["all"]:
            ...
            location_name = location_to_str_name(
                room_data, self.entity_to_place_map["receptacles"][r_id]
            )
    ...
    def build_lifted_class_matrix(self) -> None:
        ...
        self.lifted_class_matrix = np.zeros(
            (len(self.objects["class_count"]), len(self.receptacles["class_count"]))
        )
